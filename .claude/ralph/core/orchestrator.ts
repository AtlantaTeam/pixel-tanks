                ...
                if (req.kind === 'new-issue') {
                    ...
                    if (anchor) {
                        ...
                        try {
                            ...
                            try {
                                // Место дописано в текст и здесь: файловый комментарий знает
                                // файл, но не строку, а исполнителю правок нужна строка.
                                taskSource.commentOnPullRequest(pr.number, {
                                    body: `${where} — ${req.comment}`,
                                    anchor: { path: anchor.path },
                                });
                                logFn(
                                    `⚠ Якорь строки ${where} форж не принял ` +
                                        `(${(lineErr as Error).message}) — строка вне изменённых ханков; ` +
                                        `замечание легло комментарием файла ${anchor.path}.`,
                                );
                            } catch (fileErr) {
                                ...
                                // Громко и с РАЗЛИЧИМОЙ причиной: молча подменённый способ
                                // доставки — «тихий дефолт» (инвариант №1), а одна фраза на
                                // две разные причины не даёт человеку понять, что чинить.
                                logFn(
                                    `⚠ Якорь ${where} форж не принял ` +
                                        `(${(lineErr as Error).message}); файла ${anchor.path} ` +
                                        `нет в диффе PR (${(fileErr as Error).message}) — ` +

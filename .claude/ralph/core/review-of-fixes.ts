...
// «ревью → правки → ревью», ни остановки на человеке ТАМ, ГДЕ СПОР РАЗРЕШИМ МАШИНОЙ.
// Отсюда лестница, которую и считает этот модуль:
//
...
//
// ГДЕ ЧЕЛОВЕК ВСЁ-ТАКИ ПОЯВЛЯЕТСЯ (#628, честная формулировка). «Стопа нет ни в одной
// ветке» — неправда, и говорить так вредно: следующий агент чинил бы не то. Терминал
// «арбитр ВОСПРОИЗВЁЛ блокер» уводит фазу в штатный разбор `blocked`, а тот после
// `blockedHealAttempts` кругов оставляет PR человеку. Это граница автономности, а не дыра:
// подтверждённый блокирующий дефект в правках — ровно тот случай, ради которого человека и
// стоит будить. Лестница гарантирует другое — что до человека доходят ТОЛЬКО такие случаи,
// а не всякое несогласие двух проходов ревью.
//
...
export type ReviewOfFixesState = {
    ...
    arbitrated: boolean;
    // Ключи находок, по которым вердикт арбитра ПОЛУЧЕН («блокирующего не воспроизвёл») —
    // и только они. Вердикт адресный: арбитр судил предъявленный ему набор находок, а не
    // «всё блокирующее, что случится в этой фазе потом». Именно этот список гасит короткое
    // замыкание `decideAfterFixReview`, а не булев флаг: флаг переживает рестарт процесса и
    // после него относится к ПРОШЛОМУ коду (#630-2, см. докблок ветки).
    arbitratedKeys: string[];
};
...
export function normalizeReviewOfFixes(value: unknown): ReviewOfFixesState {
    ...
    return {
        ...
        arbitratedKeys: keys(v.arbitratedKeys),
    };
}
...
export function classifyFixReview(
    ...
    return {
        ...
        next: {
            ...
            arbitratedKeys: [...state.arbitratedKeys],
        },
    };
}
...
export function decideAfterFixReview(
    ...
    // Находки, которых вердикт арбитра НЕ касается: он их либо не видел вовсе, либо видел и
    // ПОДТВЕРДИЛ (тогда ключи на диск не пишутся — orchestrator.ts). Ровно они и держат
    // мердж дальше; всё остальное короткое замыкание ниже гасит.
    const judged = new Set(next.arbitratedKeys);
    const unjudged = blocking.filter((item) => !judged.has(item.key));
    if (next.arbitrated && !unjudged.length) {
        // Арбитр уже высказался по ЭТИМ находкам. Второй раз его звать не о чем — а держать
        // фазу дальше значит вернуть ровно тот стоп, ради отмены которого он и заведён.
        //
        // ПОЧЕМУ СВЕРКА ПО КЛЮЧАМ, А НЕ ПО ФЛАГУ (#630-2). Флаг `arbitrated` живёт на диске,
        // а `submitted` выставляется ПОЗЖЕ него (заведение карточек, синк доски, пуш в
        // Telegram — orchestrator.ts). Смерть процесса в этом окне оставляет на диске
        // «арбитр отработал» при `submitted === false`, и рестарт прогоняет ВЕСЬ цикл сдачи
        // заново: новое ревью, новая сессия правок — то есть НОВЫЙ код, которого арбитр
        // никогда не видел. Пока условием был флаг, свежая блокирующая находка по этому коду
        // уходила в мердж: сначала на первом круге (первая редакция #630), а после сужения
        // до `!freshBlocking.length` — на втором, где она становится «повторной» (её ключ
        // лестница сама записала в `answered` кругом раньше). Обход не исчезал, а сдвигался
        // на круг: арбитр не звался, карточка не заводилась, находка пропадала совсем.
        //
        // Поэтому гасим ИМЕННО разрешённый спор: находку, по которой вердикт «не
        // воспроизвёл» получен, — сколько бы раз ревью её ни повторяло. Находка, которой
        // арбитр не судил, проходит барьер при любом состоянии на диске: её разбирает круг
        // правок, а на исчерпании потолка — арбитр. Проверка здесь, а не в оркестраторе
        // (обнуление лестницы на входе в сдачу), намеренно: fail-closed не должен зависеть от
        // того, кто и когда чистит state (инвариант №1). Цена — повторный вызов арбитра
        // после падения; он ограничен теми же потолками.
        //
        ...
        // `repeatedBlocking` (мердж вот этой, если ключ ещё и в `arbitratedKeys`). Окно
        // узкое — нужен другой дефект в том же файле с совпадающей формулировкой, — но оно
        // есть, и потому обнуление лестницы (`clearFixReviewLadder`) остаётся ОБЯЗАТЕЛЬНОЙ
        // гигиеной, а не украшением. Читать это как «состояние на диске на исход больше не
        // влияет» нельзя.
        return {
            action: 'merge',
            reason: 'независимый арбитр по этим находкам уже вынес вердикт — второй раз петлю не крутим',
        };
    }
    ...
    return {
        ...
        reason: blocked
            ...
            : `блокирующих находок в правках: ${String(unjudged.length)}`,
        blocking: unjudged,
    };
}
...
//
// Первая редакция (#625) снимала все метки роутинга целиком, и цель была верной: в этом
// проекте `backlogLabels` подобраны под косметику, там `complexity:low` = самая слабая
// («механическая») модель, а спорную карточку по определению держит blocker или major,
// который не развели два прохода ревью и арбитр. Механической модели там делать нечего.
//
// Но голое снятие платит дважды. Карточка остаётся БЕЗ метки сложности вовсе — то есть
// нарушает конвенцию трекера там, где та требует ровно одну `complexity:*` и одну
// `area:*`, — а роутинг для неё превращается в `modelRouting.default`, который сильным
// быть НЕ обязан. Замена сильнейшей меткой достигает исходной цели и не создаёт ни того,
// ни другого.
//
...
export function disputeLabelsFor({
    ...
    for (const label of backlogLabels) {
        if (!isRouting.has(label)) {
            out.push(label);
            continue;
        }
        // Роутинговых меток в базе может быть несколько — сильнейшая ставится ОДНА, на

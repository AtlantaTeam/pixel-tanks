...
// ── Ревью фазы (#363, трек «Фреймворк ralph», фаза 3) ─────────────────────────
// pickReviewModel / pickReviewFallbackModel / reviewModelRank / strongerReviewModel /
// assertKnownReviewModels / recordReviewFindings (+ REVIEW_MODEL_STRENGTH) вынесены в
// review.ts тем же приёмом, что gate.ts (#362): фабрика createReviewModule захватывает
// контекст ralph.js (ленивый config через getConfig, ghJson, sh/shArgv/shq, log, fail,
// isPlainObject и уже определённые выше matchRiskPaths/phaseDiffFiles) один раз, а
// возвращённые функции сохраняют показательную DI (cfg/ghJsonFn/shFn/… параметрами) —
// ровно так их зовут ре-экспорт module.exports и тесты (ralph.test.js). Поведение НЕ
// меняется. Читаются как const из деструктуризации ниже.
//
// getConfig, а не config напрямую: module-level config заполняется в main() ПОСЛЕ
// загрузки модуля, поэтому дефолт pickReviewModel/pickReviewFallbackModel `cfg = config`
// обязан читать ЖИВОЙ config в момент вызова (ленивый геттер), а не снимок undefined с
// момента сборки фабрики — тот же приём, что state-lock.ts.
const { createReviewModule } = require('./review.ts');
const {
    pickReviewModel,
...
// #362 (трек «Фреймворк ралф», фаза 3): исполнение гейта мерджа фазы — последовательность
// шагов (checksGreen + состав BASE/PROD_GATE_CHECKS + gateChecksFor), логика перехода в
// blocked/hold (tryMergePhase) и детерминированные помощники метки blocked
// (removeBlockedLabel/addBlockedLabel разбора #217/#223), а также проверка «фаза уже
// смерджена» (phaseMerged/mergedPhasePr) — вынесены в gate.ts. Фабрика захватывает контекст
// ralph.js (sh/shArgv/shq/log/ghJson, хореографию findOpenPr/ensureClean/park/обновление
// дерева, санацию env чеков, sleep, DRY, regex'ы) один раз; возвращённые функции сохраняют
// показательную DI и читаются здесь как const из деструктуризации — тесты (ralph.test.js,
// blocked-scenarios.test.js, hold-scenarios.test.js) зовут их через тот же ре-экспорт из
// module.exports, что раньше. Состав шагов ПОКА хардкод в модуле (в конфиг — фаза 4, #204).
...
// как раньше module.exports у ralph.js.
const { createGateRunner } = require('./gate.ts');
const {
    BASE_GATE_CHECKS,
    PROD_GATE_CHECKS,
    gateChecksFor,
...
// #364 (трек «Фреймворк ralph», фаза 3): деплой-проверка фазы — плейсхолдер маркера
// деплоя (deployPhasePlaceholder), sha squash-мерджа (mergedShaOf), поллинг deploy-workflow
// (waitForDeployRun/deployWaitMessage), HTTP-healthcheck прода (probeHttpStatus/
// checkProdHealth) и классификация итога (isWorkflowGreen/classifyDeployOutcome) — вынесены
// в deploy-check.ts тем же приёмом, что gate.ts (#362) и review.ts (#363). Фабрика захватывает
// контекст ralph.js (ленивый config через getConfig, ghJson, shq, log, sleep, guardSideEffect,
// positiveIntOrDefault, SHA40_RE) один раз, а возвращённые функции сохраняют показательную DI
// и читаются здесь как const из деструктуризации — тесты (ralph.test.js) зовут их через тот же
// ре-экспорт module.exports. Поведение НЕ меняется.
const { createDeployCheckModule } = require('./deploy-check.ts');
const {

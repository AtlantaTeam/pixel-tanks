...
const BASE_GATE_CHECKS = [
    ...
    // ослабшей проверке» (кто-то удалил/выключил тесты, покрытие формально держится) —
    // порог coverage (#82) этого не ловит. Стоит ПЕРВЫМ: `vitest list` (сбор без прогона)
    // секундный, а следом идут минутный build и полный `test` — fail-fast от дешёвого к
    // дорогому, красный храповик отменяет мердж, не оплатив их.

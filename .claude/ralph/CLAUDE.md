...
`looksBlind`) и fail-closed сверяет, что в набор попал каждый из 19 файлов ядра — 17 TS-модулей
(включая ESM-`gate-env.mts`), рантайм-`monitor.js` и entry-`ralph.js` — плюс `scripts/project-sync.mjs`.

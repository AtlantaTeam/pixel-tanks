...
13. **Ревью правок не имеет права стать вторым стопом** (#625). Правки по ревью — код,
    ...
    **Где человек всё-таки появляется** (#628): терминал «арбитр ВОСПРОИЗВЁЛ блокер» ведёт
    ...
    замыкание гасит спор ПОИМЁННО — только находки из `arbitratedKeys`, чьи ключи легли на
    диск вместе с полученным вердиктом «не воспроизвёл». Булев `arbitrated` решением больше
    не управляет: по нему замыкание гасило любую блокирующую находку после рестарта, и
    сужение «только свежие проходят» дефект не убирало, а сдвигало на круг — на втором
    круге та же находка становится повторной и уходила в мердж без арбитра и без карточки.
    Ровно настолько, не шире: находка, которую арбитр не судил, держит мердж при любом
    состоянии на диске, но «свежесть» решает дедуп (`answered`/`settled`), а он привязан к
    ПРОШЛОМУ коду — совпал ключ находки, и новый дефект зачтётся повтором. Поэтому обнуление
    лестницы остаётся обязательной гигиеной, а не украшением, а формулировки «состояние
    живёт ровно один цикл сдачи» и «состояние на диске на исход не влияет» одинаково неверны
    — следующий агент чинил бы не то.

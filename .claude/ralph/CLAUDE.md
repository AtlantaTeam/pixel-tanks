...
tunnel-check), `adapters/` (adapters, adapters-impl), `shared/` (ralph-util,
side-effect-guard, config-profile), `runtime/` (monitor, deadman, telegram-notifier),
`tests/` (сценарные и кросс-модульные сьюты + `test-setup.js`/`test-helpers.js` +
...
`provision/`. **`gate-env.js` + `gate-env-allowlist.json` тоже в корне** (решение #396): пара
«модуль + его allowlist» держится вместе, а `DEFAULT_ALLOWLIST_PATH` (`__dirname`-провенанс,
allowlist неподделываем из дерева PR) остаётся тривиальным — увоз в `core/` дал бы только
переписанный путь без выгоды. Барьер чистоты `tests/core-purity.test.ts` после переезда
сканирует подпапки РЕКУРСИВНО (иначе grep-guard ослеп бы на пустом наборе — класс
`looksBlind`) и fail-closed сверяет, что в набор попал каждый из 14 TS-модулей ядра.
...
| `core/exec.ts`                                  | Примитивы исполнения и лога (#365): `log`/`fail`/`setLogTarget`, `sh` (чтения #133) / `shArgv` (мутации #193/#252), `ghJson` (ретраи+backoff M3), `loadJson`                                                                            | `sh`/`shArgv` под общим `guardSideEffect` #138; `log` знает текущий `logTarget`; `ghJson` — только ЧТЕНИЕ                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `shared/config-profile.ts`                      | Профили конфига (#71→#365): `deepMerge` (`common` → профиль), `parseProfileFlag`, `resolveProfile`, валидация `haltBeforeDeploy`                                                                                                        | fail-closed на любом изъяне схемы; запрещённые ключи `__proto__`/… (скан всей глубины, ярлык пути по блоку); профиль не дублирует `common`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `runtime/monitor.js`                            | Панель прогресса + deadman-пуш с дедупом по mtime лога                                                                                                                                                                                  | Read-only; одно чтение лога на тик; `logFn: console.log` у пуша (не `log()` раннера)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `runtime/deadman.js`                            | Чистые правила: классификация хвоста лога → порог тишины по режиму                                                                                                                                                                      | Без побочек вовсе; режим `stopped` = порог Infinity; сверка с `apiLimitMessage`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `runtime/telegram-notifier.js`                  | Отправка в TG, fail-open, anti-RCE argv, токен вне argv                                                                                                                                                                                 | Самостоятельный модуль (не require ralph.js — цикл); предохранитель и журнал — общие из `side-effect-guard.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `shared/side-effect-guard.ts`                   | Общий предохранитель #138 (`NO_SIDE_EFFECTS`/`sideEffectAttempts`/`guardSideEffect`) на все три потребителя                                                                                                                             | TS без билд-шага; один журнал на ralph.js/telegram-notifier.js/security-audit.mjs (кеш модуля Node отдаёт тот же массив); `hint` — параметр, у потребителей разные DI                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `tests/test-setup.js` / `tests/test-helpers.js` | Общий afterEach предохранителя #138 на весь vitest-проект `ralph`                                                                                                                                                                       | Сверяет единый журнал `side-effect-guard.ts` (один на все три модуля); не переносить обратно в один тест-файл                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |

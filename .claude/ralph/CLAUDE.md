...
13. **Ревью правок не имеет права стать вторым стопом** (#625). Правки по ревью — код,
    ...
    **Где человек всё-таки появляется** (#628): терминал «арбитр ВОСПРОИЗВЁЛ блокер» ведёт
    ...
    И ещё одно, из той же карточки: **состояние лестницы живёт ровно один цикл сдачи**.
    Новый цикл (`--resubmit`, чини-сессия гейта) начинает с `reviewOfFixes = null`, а
    «арбитр отработал» пишется на диск ТОЛЬКО после полученного вердикта. Иначе heal-коммит
    (новый код!) судился бы унаследованным `arbitrated: true` — то есть уезжал бы в main
    непрочитанным.

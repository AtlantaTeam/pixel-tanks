...
4. **Гейт детерминирован и не верит агенту на слово.** Мердж только если: нет открытых
   ...
   PR без label `blocked`, локальный ref ветки == голова PR (H3), все чеки зелёные на
   detached checkout **точного sha** PR-головы, и `gh pr merge --match-head-commit <sha>`
   закрывает TOCTOU-окно между чеками и мерджем. Порядок прод-чеков — fail-fast от
   дешёвого к дорогому (security → coverage → e2e): не переставлять.
...
- Тот же класс в разборе blocked (#217): чини-сессия сама снимала label blocked и
  ...
  кодер-сессии. Раннер сам снимает метку и гоняет ПОВТОРНОЕ ревью (`removeBlockedLabel`
    - инлайн re-review в ветке `gate === 'blocked'`), метку возвращает ревью, если блок
      устоял. Планка `strongerReviewModel`/`reviewModelRank` держит модель повторного ревью
      не слабее поставившей блок (`state.reviewModelFloor`) — иначе эскалацию обходят
      удешевлением ревьюера (haiku после блока от fable).

...
карта фреймворка и контракты адаптеров — `README.md`; как запускать и на какие грабли наступали — `RUNBOOK.md`;
правила для кодер-сессии внутри петли — `ralph.md`. Здесь — архитектура, инварианты и границы переносимости.
...
| `gate-env.mts`                                  | Санация переменных окружения гейта (#188): DRY-свеженость, список разрешённых, экспорт структурированных env для чеков                                                                                                                                                                                                | ESM-файл (`.mts`) ради `import.meta.dirname` (#403) — читает allowlist из launch-дерева (`DEFAULT_ALLOWLIST_PATH`), не из worktree; путь прошит в документации как важный инвариант (провенанс)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
всем 20 файлам ядра пуст вне комментариев — сверка в `tests/core-purity.test.ts`).

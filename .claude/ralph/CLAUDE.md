...
| `adapters.ts`                         | **Интерфейсы** пяти швов (#262): `TaskSourceAdapter`/`GateAdapter`/`NotifierAdapter`/`DeployCheckAdapter`/`CoderRuntimeAdapter` + `RalphAdapters`. Только типы (erasable), реализаций нет                                               | режим отказа (fail-closed/open) — в докстроке, часть контракта; не импортирует боевые модули (зависимость «абстракция → реализация», не наоборот)                                                  |
| `adapters-impl.ts`                    | **Сборка** швов (#369): мапперы боевых функций → интерфейсы (`createGithubTaskSource`/`createNpmGate`/…), реестр реализаций, `buildAdapters` + `resolveAdapterSelection` (выбор по `config.adapters`, fail-closed)                      | значения методов — те же боевые ссылки (поведение прежнее); неизвестный шов/имя/незарегистрированная реализация = `fail`; ядро зависит только от интерфейсов, composition root в `orchestrator.ts` |

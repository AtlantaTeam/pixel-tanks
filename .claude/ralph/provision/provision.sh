...
echo "Штатный запуск ralph (управляемый процесс, переживает разрыв SSH — RUNBOOK.md, «Запуск»):"
echo "  $REPO_DIR/.claude/ralph/provision/start-tmux.sh prod        # вариант A: tmux + дашборд"
echo "  sudo cp $REPO_DIR/.claude/ralph/provision/ralph.service /etc/systemd/system/ \\"
echo "    && sudo systemctl daemon-reload && sudo systemctl start ralph   # вариант B: systemd (AFK)"
echo "Разовый/тестовый прогон вручную (из КОРНЯ репо — пути в ralph.js относительные):"
echo "  cd $REPO_DIR && set -a && . $ENV_FILE && set +a && node .claude/ralph/ralph.js --profile prod --dry-run"

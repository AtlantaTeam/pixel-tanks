...
```bash
cd /root/pixel-tanks && node .claude/ralph/ralph.js --dry-run   # cwd НЕ переключается (read-only)
...
**Если `package.json` в ветке фазы добавил новую зависимость** — `npm ci` раннер
гоняет только один раз, при создании worktree; гейт-чеки после этого могут упасть на
отсутствующем пакете. Почини руками: `cd /root/pixel-tanks-ralph && npm ci`.

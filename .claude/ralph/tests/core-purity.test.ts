...
describe('набор сканируемых модулей ядра', () => {
    ...
    // из скана и этот список его недосчитается. Перечислены все 22 файла ядра: 20 TS-модулей
    // (12 core + 2 adapters + 2 shared + 3 runtime + ESM-gate-env.mts; runtime включает
    ...
    it('непуст и включает КАЖДЫЙ ожидаемый модуль ядра', () => {
        // Явный `EXPECTED` строго сильнее прежнего `CORE.length > 10`: если каждый из 22
...
describe('ядро не исполняет команд форжа (#55)', () => {
    ...
    const FORGE_CMD =
        /(\bgh\s+(pr|issue|api|auth|run|project)\b)|(\(\s*['"`]gh['"`]\s*,)|(\bglab\s+[a-z])/;
    ...
    it.each(CORE_MODULES)('%s — без команд gh/glab в исполняемом коде', (rel) => {
        ...
        const detail = hits.map(([n, line]) => `\n  ${n}: ${line.trim()}`).join('');
        expect(hits, `Команда форжа в ядре ${rel} — её место в adapters/:${detail}`).toEqual([]);
    });
});

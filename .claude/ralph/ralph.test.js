...
describe('#167: пост-мердж проверка — критерии готовности (тайминг, ретраи, стоп+пуш)', () => {
    const SHA = 'a'.repeat(40);
    // Детерминированные часы: nowFn читает clock, sleepFn его двигает. Так все
    // утверждения о «времени» (не дольше таймаута, ограниченное число опросов)
    // проверяются без реального сна.
    const mkClock = () => {
        const c = { t: 0 };
        return {
            clock: c,

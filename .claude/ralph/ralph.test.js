...
describe('runLoop — основной while-цикл: итерации кодера, сдача, гейт, self-heal (#104)', () => {
    ...
    // #151: живость монитора проверяется на КАЖДОМ проходе while-цикла, не только
    // на старте. Сценарий с continue (фаза уже смерджена → advancePhase → 2-й проход
    // «за концом» → break) даёт ровно 2 прохода цикла — оба обязаны проверить монитор.
    it('#151: ensureMonitorAlive зовётся на каждом проходе цикла (AFK, не только на старте)', () => {
        const logs = [];
        ...
        expect(ensureMonitorAliveFn).toHaveBeenCalledTimes(2);
    });

    it('#151: ensureMonitorAlive получает профиль и путь конфига раннера', () => {
        const logs = [];
        const ensureMonitorAliveFn = vi.fn(() => null);
        runLoop(

    ...
    return (
        <div
            className={clsx(
                ...
                // Порог 1280, а НЕ `xl` (у нас 1200, `--breakpoint-xl` в globals.css):
                // бюджет ширины освободился не на всём xl (ревью #566). Замер на ветке
                // (dev, /game?seed=42): при `xl:px-1` полоса `top-hud-desktop` на 1200
                // переполнена на 39px, на 1232 — на 7px, с 1248 укладывается; сами 4
                // видимые ячейки xl стоят 32px этого бюджета. Часть переполнения
                // (≈7–24px) остаётся и с нулевым паддингом — это отдельный дефект
                // левого кластера, заведён карточкой #592. Поэтому
                ...
                // ячейки идут плотными ВСЕ разом, без разнобоя.
                compact ? 'px-0 py-0' : 'px-2 py-1 xl:px-0 xl:py-0 min-[1280px]:px-1',
                className,
            )}
        >

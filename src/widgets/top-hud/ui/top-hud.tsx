    ...
    return (
        <div
            className={clsx(
                ...
                // в ряду стояли ОБЕ карточки — «Снаряды» и «Ходы». С уходом «Снарядов»
                // с xl (#548, `xl:hidden`) бюджет освободился, и обычный паддинг в него
                // влезает: замер на 1280 — ряд телеметрии 377px при доступных 410px,
                // `scrollWidth === clientWidth`. Разнобой же (0px у ресурсов против 4px
                // у Угла/Силы/Ветра) читался на 1440/1920 как поломка вёрстки.
                compact ? 'px-0 py-0' : 'px-2 py-1 xl:px-1 xl:py-0',
                className,
            )}
        >
                ...
                >
                    ...
                    >
                        {/* Планшет/десктоп (#528): та же `CellShell`-карточка (рамка +
                            ...
                            затем (#528/#489) единственной парой без паддинга. Барьер
                            равенства паддингов и укладки ряда (#566) —

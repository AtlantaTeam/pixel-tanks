    ...
    return (
        <div
            className={clsx(
                ...
                // На 768–1279 (#538, ужато #592) паддинг карточки — p-1.5: бюджет
                // ширины узкого xl (1200–1279, полоса 78px без запаса) не оставляет
                // места под исходный p-2 — левый кластер ряда статуса (HP-карты +
                // пилюля + бейдж + иконки) переполняет полосу на 7px при p-2 ровно
                // на 1200 (#592). С xl-wide (1280, полоса с запасом — тот же порог,
                // что у паддинга ячеек телеметрии, `CellShell`, #566) паддинг
                // возвращается к каноничному p-2. Порог `xl-wide`, а не `xl` (у нас
                // 1200) — та же ловушка «xl ≠ 1280», что чинил #566.
                ...
                layout === 'inline' ? 'px-1 py-1' : 'p-1.5 xl-wide:p-2',
        ...
        >
            <HPBar
                ...
                max={MAX_HP}
                layout={layout}
                // Плотность выбирает ВИДЖЕТ, а не компонент (#561, ревью PR фазы):
                // ужатые зазоры нужны там, где строка HP делит ряд с пилюлей хода,
                // бейджем и кнопками, — то есть в обеих inline-раскладках боевого
                // HUD (мобилка и планшет 768–1279). Канон `comfortable` остаётся в
                // `shared/ui` и на витрине.
                density={layout === 'inline' ? 'compact' : undefined}
                hitNonce={hitNonce}
            />
        </div>
    );
...
/** Размерник ширины ячейки «Угол»: три знака. Само значение «360°» недостижимо —
 *  показ угла нормализован в [0, 360) (#457), фактический потолок «359°». Размерник
 *  всё равно берём трёхзначный: цифры табличные (`tabular-nums`), поэтому ширина у
 *  «360°» и «359°» одна, а именованная константа не даёт следующему читателю искать,
 *  откуда взялся недостижимый максимум (ревью #457). */
const ANGLE_VALUE_SIZER = '360°';
    ...
    return (
        ...
        <div className="flex shrink-0 items-center gap-2">
            <Button
                variant="ghost"
                size="icon"
                // `m-0` гасит базовый `m-1` компонента `Button`: без него кнопки
                // mute/пауза получали паразитные 4px margin со всех сторон
                // (44+8=52px вместо 44) и ряд 1 HUD не укладывался в бюджет высоты
                // 44px. Раньше здесь стоял костыль `!m-0` (#538) — простой `m-0` не
                // побеждал, потому что `buttonClasses` склеивал строки через `clsx`
                // и каскад решал порядок в генерируемом CSS. С переходом
                // `buttonClasses` на `tailwind-merge` (#554) `m-1` из строки просто
                // выкидывается, и important больше не нужен.
                className="m-0"
                onClick={toggle}
            ...
            <Button
                variant="ghost"
                size="icon"
                // `m-0` — см. комментарий у кнопки звука выше (#554): важен сам
                // факт гашения базового `m-1`, important для этого не нужен.
                className="m-0"
    ...
    return (
        <div
            className={clsx(
                ...
                // Отдельного «без паддинга» режима у ячеек ресурсов больше нет (#566):
                // флаг `tight` заводился под бюджет ширины узкого xl (#528/#489), когда
                // в ряду стояли ОБЕ карточки — «Снаряды» и «Ходы». Разнобой (0px у
                // ресурсов против 4px у Угла/Силы/Ветра) читался на 1440/1920 как
                // поломка вёрстки. Теперь паддинг у всех ячеек ряда общий — плотный
                // режим включается не по роли ячейки, а по ширине кадра.
                // Порог `xl-wide` (1280), а НЕ `xl` (у нас 1200, `--breakpoint-xl` в
                // globals.css): бюджет ширины освободился не на всём xl (ревью #566).
                // Замер на ветке (dev, /game?seed=42): при `xl:px-1` полоса
                // `top-hud-desktop` на 1200 переполнена на 39px, на 1232 — на 7px, с
                // 1248 укладывается; сами 4 видимые ячейки xl стоят 32px этого бюджета.
                // Часть переполнения (≈7–24px) остаётся и с нулевым паддингом — это
                // отдельный дефект левого кластера, заведён карточкой #592. Поэтому
                // паддинг возвращаем с 1280 — ширины, на которой бюджет замерен с
                // запасом (ряд телеметрии 377px при доступных 410px), а на 1200–1279
                // ячейки идут плотными ВСЕ разом, без разнобоя. Порог — токен темы
                // `--breakpoint-xl-wide`, а не `min-[1280px]` (ревью #566): пятый
                // брейкпоинт проекта обязан жить вместе с остальными четырьмя.
                compact ? 'px-0 py-0' : 'px-2 py-1 xl:px-0 xl:py-0 xl-wide:px-1',
                className,
            )}
        >
 ...
 *
 * Подпись — три ступени по ширине (#561 и ревью PR фазы). Полная «Заморожено» с
 * `xl` (1200), где полоса уже не в дефиците. Короткая «Замор.» на 1024–1199:
 * полное скрытие текста оставляло зрячего игрока с одним замком 12px без
 * текстовой опоры (узел `aria-hidden` целиком — анонс для AT даёт отдельный
 * `FreezeAnnouncer`), а на этих ширинах ник HP-карты уже не клипается и место
 * под сокращение есть. Ниже 1024 — только замок: там ряд 1 отдаёт каждый
 * пиксель нику (handoff «HP обязан схлопываться последним»), и подпись —
 * даже сокращённая — возвращает клип короткого ника («Terminator»), то есть
 * прямо откатывает #561; замерено e2e на 768. Ступени именно такие, а не
 * `md`/`xl`: ниже 1200 состав бейджа тоже рендерится (раскладка md+), и порог
 * `md` был бы шире собственного объяснения.
 */
function FreezeBadgeOrNothing({ visible }: { visible: boolean }) {
    return (
        ...
        >
            ...
            {/* Две формы подписи, а не одна с обрезкой: сокращение — смысловое
                («ЗАМОР.» читается словом), а не многоточие посреди слова. */}
            <span className="hidden font-ui text-[10px] tracking-[0.1em] whitespace-nowrap text-text-muted uppercase lg:inline xl:hidden">
                Замор.
            </span>
...
export function TopHud({ onPauseClick }: TTopHudProps = {}) {
    ...
    return (
        ...
        >
            ...
            >
                {/* xl:gap-1 (#489): базовый зазор ряда — gap-2 (был gap-4 до #561,
                    см. комментарий на самом ряду), на узком xl тот же бюджет ширины —
                ...
                <div
                    ...
                    // gap-2 (было gap-4, #561): на 768–1199 ряд отдаёт нику
                    // HP-карточки те же 8px — по handoff «HP-карточка» HP
                    // схлопывается последним. На xl зазор и так переопределён ниже.
                ...
                >
                    ...
                    >
                        {/* Подпись — всегда «Угол» без суффикса «заморожен» (#473):
                            ...
                            `ANGLE_VALUE_SIZER`/`POWER_VALUE_SIZER` держит ширину бокса
                            значения неизменной. */}
                        <NumberCell
                            label="Угол"
                            value={angleValue}
                    ...
                    >
                        {/* Планшет/десктоп (#528): та же `CellShell`-карточка (рамка +
                            подложка) и тот же паддинг, что у Угла/Силы/Ветра — раньше
                            снаряды и ходы были единственной парой ячеек без карточки, а
                            затем (#528/#489) единственной парой без паддинга. Плотный
                            режим на узком xl (1200–1279) остался, но общий для всего
                            ряда, а не для одной пары (см. `CellShell`). Барьер равенства
                            паддингов и укладки ряда (#566) —
                            `e2e/hud-resource-cell-padding.spec.ts`. */}
                        {/* «Снаряды» скрыты на xl (#548, разбор §2/§3): на десктопном
                            составе (xl, то есть с 1200 — `--breakpoint-xl` в globals.css,
                            НЕ с 1280) селектор палубы уже показывает боезапас выбранного
                            оружия («Фугас ×2», `game-controls.tsx`) — ячейка дублирует
                            эту цифру и занимает ~90px в самом тесном месте полосы.
                            На планшете (768–1199) палуба показывает то же самое, но
                            там ячейка остаётся — бюджет ширины там не жмёт так, как на
                            узком xl (#489). Числа в этом комментарии раньше говорили
                            «≥1280»/«768–1279» и расходились с классом (ревью #566) —
                            ровно та ловушка «xl ≠ 1280», которую чинит #566 двумя
                            блоками выше, в `CellShell`. `xl:hidden` (не размонтирование):
                            ячейка не несёт своего состояния, спрятать классом дешевле и
                            не меняет поведение ниже xl. */}
                        <CellShell label="Снаряды" className="xl:hidden">
                            <ResourcePipVisual
                                pips={ammoPips}
                                color="var(--color-accent)"
                                ariaLabel="снарядов"

 ...
 *
 * Значения по брейкпоинтам: <768 — 242/196, ≥768 (md) — 128/168, ≥992 (lg) —
 * 128/124, ≥1200 (xl) — 98/124.
 *
 ...
 * md-инсет 128.
 *
 * ⚠️ «Ровно фактическая высота» — это про xl и только про него (ревью #557):
 * там обе полосы фиксированы (HUD `xl:h-[78px]`+20 паддинга = 98, дека
 * `xl:h-[124px]` = 124), и инсеты совпадают с ними до пикселя. Остальные
 * значения — ЗАПАС В БЕЗОПАСНУЮ СТОРОНУ поверх фактической высоты, потому что
 * там полосы content-sized и растут от контента (перенос ряда, safe-area на
 * мобиле). Замер на dev (`/game?seed=42`, высота 900): 390 — низ HUD 168 при
 * инсете 242, дека 153 при инсете 196; 768/1024 — дека 82 при инсетах 168/124.
 * Перекрытия нет ни в одном кадре — зона лишь меньше теоретического максимума.
 * Поэтому «подгонять» числа под замер одного вьюпорта нельзя: запас держит
 * кадры, где полоса выше (узкие телефоны, safe-area, длинный ник).
 *
 ...
 * Расхождение ловит e2e `gesture-zone-hud-budget.spec.ts` — и сверху (низ HUD vs
 * верх зоны), и снизу (низ зоны vs верх палубы, ревью #557); прежние e2e ловили
 * лишь горизонтальный overflow и видимость лока, но НЕ вертикальное
 * выравнивание зоны.
 */

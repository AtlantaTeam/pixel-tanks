...
/**
 * Верхняя оценка радиуса очага, по которой считаются габариты полосы.
 *
 * Пиков на самом деле ДВА, и `+growthPerFrame` — про второй из них.
 * НАРИСОВАННЫЙ радиус максимума не достигает вовсе: `paintExplosionFocus` зовётся
 * ДО инкремента, значит последний нарисованный кадр строго меньше
 * `radiusFactor · baseRadius`. А ВОРОНКА режется уже по инкрементированному
 * радиусу, то есть перешагивает максимум почти на целый шаг — вот её пик здесь и
 * заложен. Для вспышки это лишний шаг запаса: полоса выходит чуть шире нужного,
 * и это осознанно — одна оценка на оба габарита дешевле двух формул, которые
 * потом разъедутся (ревью #601).
 */

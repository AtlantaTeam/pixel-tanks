...
export class Ground {
     ...
     *
     * Числа, а не объект: геттер читается каждый кадр, а `canvas.md` просит не
     * аллоцировать в кадре.
     */
    fallingFrom = -1;
    fallingTo = -1;
    ...
    /**
     * Ровная полоса высот — рельеф демо-кадров витрины и тестов, где дюны только
     * мешают (демо флажка ветра: наклон корпуса не должен спорить с наклоном
     * полотнища). `risePerColumn` задаёт постоянный уклон: ноль — плоскость,
     * ненулевой — ровный склон, на котором корпус получает наклонный трансформ
     * (`slopeTank`) и проверяется геометрия всего, что живёт вне него (флажок, #579).
     *
     ...
     * закешированный старый профиль. Заодно сбрасываются воронки: ровный рельеф
     * с чужими кратерами — не ровный рельеф.
     */
    flatten = (height: number, risePerColumn = 0) => {
        this.heights = Array.from({ length: this.innerWidth }, (_, x) =>
            floor(height + x * risePerColumn),
        );

...
export type TBattleStats = {
    /**
     * Урон противнику: `MAX_HP − enemyHp`, зажат в 0..MAX_HP. Это падение HP
     * бота от ЛЮБОГО источника, а не только от попаданий игрока — редкий самострел
     * бота (в него же прилетел его снаряд) тоже учтётся как «урон». В фазе 1 это
     * принятое упрощение: отдельный счётчик нанесённого игроком урона заведём с
     * тюнингом урона/оружия (GDD §9-E, мультиоружие). Точность самострелы бота НЕ
     * задевает — она считает только попадания игрока (`recordPlayerHit`).
     */
    damage: number;
 ...
 * Всегда неотрицательное целое; жёсткий потолок формулы `3·MAX_HP` вынесен в
 * `MAX_LEADERBOARD_POINTS` (`shared/config`) — он же серверная граница валидации
 * записи `scores`/`submitDailyScore`, чтобы окно накрутки было равно диапазону
 * формулы, а не на 2.5 порядка шире.
 */

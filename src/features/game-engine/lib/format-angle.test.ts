...
describe('formatAngle', () => {
    ...
    it('нечисловой вход показывается как 0°, а не «NaN°»', () => {
        // Докблок обещает это отдельно (NaN — сентинел в тестах движка), но держится
        // обещание на цепочке `% 360` + `| 0`: замена усечения на `Math.floor` при
        // чтении выглядит невинной чисткой и молча вернула бы «NaN°» в HUD (ревью #457).

...
const CELESTIAL_Y_RANGE: Record<TSkyPresetId, readonly [number, number]> = {
    day: [0.26, 0.4],
    // Хотели бы 0.56, но у заката ниже не пускает горизонт: диск крупный, и
    // `maxCelestialYFrac` всегда выбирает потолок по линии гор (тест «у заката
    // потолок ВСЕГДА горизонтный»). Число здесь — намерение; действующее правило
    // считает арифметика, а не эта константа.
    sunset: [0.4, 0.56],
    night: [0.26, 0.42],
};

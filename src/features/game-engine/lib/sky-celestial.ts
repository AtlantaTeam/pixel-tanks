...
/**
 * Сектор Y по пресету: день — высоко в небе, закат — низко у горизонта (ПОДОШВА
 * силуэта гор стоит на 0.62 высоты, `sky-scene.ts`, — солнце уходит за них
 * частично), ночь — луна выше горизонта.
 *
 ...
 * хребтом (#583).
 *
 * **У заката 0.56 недостижимо — число оставлено как намерение, а не как правило.**
 * `horizonLimit = 0.62 − radiusFrac − 0.01`, а закатный радиус ∈ `[0.075, 0.095]`,
 * значит потолок там ∈ `[0.515, 0.535]` и `Math.min` всегда выбирает горизонт (тест
 * «у заката потолок ВСЕГДА горизонтный» это и держит). Правишь диапазон радиусов —
 * опирайся на арифметику `maxCelestialYFrac`, а не на это число: оно вступит в силу
 * только если закатный диск станет заметно мельче (ревью #601).
 *
 ...
 * ниже про закат.
 *
 * Оговорка про `Math.max`: он fail-open. Сработал — значит сектор и диапазон радиусов
 * разъехались (`horizonLimit` провалился под `yMin`), и инвариант «диск не ниже гор»
 * молча перестал держаться. Чинить в этом случае надо ДИАПАЗОНЫ, а не потолок; сегодня
 * ветка недостижима (`horizonLimit ≥ 0.515 > yMin = 0.4` на всех пресетах), и покраснеет
 * при регрессе перебор по 400 сидам, а не тест самой функции.
 *

 ...
 * (`y` от 0 до `THICKNESS`), как настоящий флаг на древке. Симметричная относительно оси
 ...
 * (`windFlagSide`, чтобы полотнище всегда провисало ПОД осью, а не задиралось
 ...
 * штиль (наклона нет). При максимуме (индекс 3) флажок перпендикулярен мачте (90°,
 * критерий #609).
 */
const LEAN_DEG_BY_MAGNITUDE: readonly number[] = [0, 30, 60, 90];
 ...
 * угла (ревью #579). Порог «угол > 90°» совпадает с направлением ровно пока наклон
 * держится в [0°, 180°]: сегодня максимум ровно на границе (`LEAN_DEG_BY_MAGNITUDE`
 * даёт 90° при максимальном ветре, то есть угол 0° вправо и 180° влево, #609).
 * Вырасти наклон дальше 90° или сменись нейтраль — зеркало по углу перевернулось бы
 * молча; знак ветра этого не замечает, поэтому сторона и берётся у него.
 */

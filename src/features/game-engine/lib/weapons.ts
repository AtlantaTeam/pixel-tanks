 ...
 * (бот). Раздача детерминирована (без random), поэтому бой и его реплей
 * получают идентичные арсеналы без записи оружия в код реплея.
 */
export const generateRandomWeapons = (amount: number = WEAPONS_AMOUNT): TTanksWeapons => {

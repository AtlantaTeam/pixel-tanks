 ...
 *  по-прежнему решает каскад — Tailwind печатает `mx-*` после `m-*`, поэтому
 *  боковые кнопки `pause-overlay` работают, но держатся ровно на том механизме,
 *  который #554 признал ненадёжным. Полностью детерминирован только случай
 *  «та же группа», то есть `m-0`.
 *

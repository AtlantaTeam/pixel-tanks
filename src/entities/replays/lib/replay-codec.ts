...
/**
 * Бинарный формат записи боя (версия 1), затем base64url без padding:
 *
 * ```
 ...
 * квантование даёт другую траекторию и ломает идентичность реплея.
 */
 ...
 * power вне u8, seed-строка длиннее 65535 байт) — такие значения в игре
 * не возникают, их появление означает баг записи, а не данные для усечения.
 */
export const encodeReplay = (replay: TReplay): string => {
    ...
    let size = 2 + (seedBytes ? 2 + seedBytes.length : 8);
...
export const decodeReplay = (code: string): TReplay | null => {
    ...
    while (offset < bytes.length) {
        ...
        if (tag === MOVE_TAG_MOVE) {
            if (offset + 2 > bytes.length) return null;
            moves.push({ kind: 'move', delta: view.getInt16(offset) });
            offset += 2;
        } else if (tag === MOVE_TAG_FIRE) {
            if (offset + 9 > bytes.length) return null;
            moves.push({ kind: 'fire', angle: view.getFloat64(offset), power: bytes[offset + 8] });
            offset += 9;
        } else {
            return null;
        }

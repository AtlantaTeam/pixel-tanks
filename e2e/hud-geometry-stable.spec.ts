...
test.describe('Ширина ячеек и координаты кнопок ± неизменны — mobile-390', () => {
    ...
    test('кнопка «+» угла не ездит при смене числа знаков значения', async ({ page }) => {
        ...
        // в зарезервированном под «360°» боксе). Шаг — 1° на нажатие, поэтому
        // накопительно (замер в браузере, dev/1280): 0 нажатий — «0°», 5 — «5°»
        // (один знак), 50 — «50°» (два), 170 — «169°» (три). Все три ширины
        // значения проверены одним сценарием.

 ...
 *  `--breakpoint-xl-wide` в globals.css). Раньше этим же порогом мерился и другой,
 *  независимый факт — «с какой ширины укладывается полоса HUD» (см.
 *  `STRIP_FITS_FROM` ниже): оба были 1280, потому что до #592 полоса не укладывалась
 *  на всём узком xl (1200–1279) даже при нулевом паддинге ячеек — левый кластер
 *  (HP-карты + пилюля + бейдж + иконки) сам переполнял её на ≈7px. #592 ужал
 *  паддинг HP-карточки (`p-1.5 xl-wide:p-2` вместо `p-1.5 xl:p-2` в `HpCard`) и
 *  снял переполнение независимо от паддинга ячеек ресурсов — с тех пор пороги
 *  разошлись, отсюда два разных имени. */
const CELL_PADDING_FROM = 1280;

/** С этой ширины полоса HUD `top-hud-desktop` обязана укладываться в свою ширину
 *  целиком (см. `expectStripFits`) — снято карточкой #592. До неё порог совпадал
 *  с `CELL_PADDING_FROM` (см. её комментарий); теперь это независимые факты. */
const STRIP_FITS_FROM = 1200;

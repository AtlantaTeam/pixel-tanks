        ...
        test('усечение ника соответствует брейкпоинту (#540, на 768 — #561)', async ({ page }) => {
            ...
            for (const box of boxes) {
                ...
                if (viewport.width >= 1024) {
                    ...
                    // после ужатия зазоров, паддинга карточки и подписи бейджа (текст
                    // «Заморожено» скрыт на планшете, остаётся иконка) бокс ника вырос с
                    // 25px до ~86px (было 104px content у «Rex Commander»). Этого хватает
                    // на короткий ник («Terminator», 80px — клипа больше нет), но не на
                    // длинный («Rex Commander», 104px — клипается на ~2 символа, а не на
                    // все 11 как раньше). Ассертим по каждому известному тексту отдельно —
                    // общей планки «клип есть всегда» больше нет, это уже не факт, а спека.
                    expect(clipped, `клип ника "${text}" на 768`).toBe(text === 'Rex Commander');
                } else {

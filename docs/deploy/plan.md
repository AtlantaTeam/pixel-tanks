...
> ✅ **Деплой выполнен** — прод живой на `https://pixeltanks.ru`. Чекбоксы ниже не отражают статус; актуальность — [доска](https://github.com/orgs/AtlantaTeam/projects/1) + milestones. Прим.: старт прод-сервера — `standalone` (см. #236), не `next start`.

...
// `test.<marker>(` / `describe.<marker>(`, включая модификаторные цепочки
// (`it.concurrent.only(`). Заякорен на начало строки после отступа (`^[[:space:]]*`):
// реальный вызов в тест-файле всегда стоит с отступа, а упоминания маркера в строковых
// литералах (`snippet: "it.skip(..."`) и комментариях (`// не пиши it.only`) — нет.
// Якорь отсекает ложные совпадения по тексту, оставляя совпадения по коду — без него
// фикстуры этих же тест-файлов красили бы гейт (ревью PR #230, 🟠 major). Переименованные
// импорты (`import { it as t }`) не ловятся — редкий для проекта случай, ESLint/конвенции
// не поощряют алиасы; на решение red/green .only это не влияет (там grep — лишь подсказка).
export function grepMarkerPattern(marker) {
    return `^[[:space:]]*(it|test|describe)(\\.[A-Za-z]+)*\\.${marker}[[:space:]]*\\(`;
}
